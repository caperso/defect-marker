...
import { FAKE_DEFECTS, FAKE_IMG_GROUP } from "../../../../store/FAKES";
import { plantStore } from "../../../../store/plantStore";
import { Defect } from "../../../../typings/interface";
...
export const DefectEditor = observer(() => {
  const images = FAKE_IMG_GROUP;
  const defectList = FAKE_DEFECTS;
  const [currentImage, setCurrentImage] = useState(images[0]);
  const [editorVisible, setEditorVisible] = useState(false);
  const [currentDefectIndex, setCurrentDefectIndex] = useState<number | undefined>(undefined);
  const { currentWayPoint, setCurrentWayPoint } = plantStore;
  ...
  // 组件监听: 当前航点,相应显示/隐藏
  useEffect(() => {
    setEditorVisible(currentWayPoint ? true : false);
  }, [currentWayPoint]);
  ...
  useEffect(() => {
    if (currentImage) {
      ...
      // ! or use ImageInfo.id?
      const defects = defectList.filter((d) => d.infrared_name === infraredName);
      setDefects(defects);
    }
    ...
    editorVisible ? (
      ...
      >
        ...
        <div className="editor-options">
          <div className="items">
            {renderDefectItems()}
            <div onClick={selectMode} className={`item ${currentDefectIndex === undefined ? "selected" : ""}`}>
              缺陷选择
            </div>
          </div>
        ...
        <div className="main">
          ...
          <div className="image-controls">
            ...
            <EditorCore
              ...
              currentDefect={currentDefectIndex === undefined ? undefined : SOLAR_DEFECT_TYPES[currentDefectIndex]}
          ...
          <textarea id="brief-editor" className="editor-brief" placeholder="备注:" />
        </div>
      </Modal>
    ) : null,
  );
